...
- **请求时间分布**：由 [TradingView Lightweight Charts](https://github.com/tradingview/lightweight-charts) 渲染的交互式时间线——绿色柱为请求量（按跨度自动选择聚合粒度：1 分钟 / 5 分钟 / 15 分钟 / 小时 / 6 小时 / 天），红色折线为错误量；支持拖动缩放、十字光标悬停查看每个时间桶的请求与错误数。十字光标的时间标签与悬停气泡均按操作者本地时区显示 `YYYY-MM-DD HH:MM:SS`；时间轴刻度随可见跨度自适应（跨天内显示 `时:分`，跨多天显示 `月/日`）。
